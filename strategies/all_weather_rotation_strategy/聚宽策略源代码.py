...
def filter_roic(context,stock_list):
    ...
    for stock in stock_list:
        ...
        if roic>0.08:
            list.append(stock)
    log.info("[filter_roic] ROIC_TTM过滤: %d -> %d 只" % (len(stock_list), len(list)))
    return list
...
def SMALL(context,choice):
    log.info("[SMALL] 输入股票数: %d" % len(choice))
    ...
    log.info("[SMALL] ROE>0.15且ROA>0.10 过滤后: %d 只" % len(df))
    ...
    log.info("[SMALL] 按市值升序后: %d 只, 取前%d只: %s" % (len(final_list), g.stock_num*3, str(final_list[:g.stock_num*3])))
    return final_list
    
def BIG(context,choice):
    log.info("[BIG] 输入股票数: %d" % len(choice))
    ...
    log.info("[BIG] 过滤+排序后取前%d只: %s" % (g.stock_num, str(BIG_stock_list)))
    return BIG_stock_list
def ROIC_BIG(context,choice):
    log.info("[ROIC_BIG] 输入股票数: %d" % len(choice))
    df = get_fundamentals(query(
            valuation.code,
        ).filter(
            valuation.code.in_(choice),
    ...
    log.info("[ROIC_BIG] 基本面过滤后: %d 只" % len(df))
    df=filter_roic(context,df)
    log.info("[ROIC_BIG] ROIC>0.08过滤后: %d 只" % len(df))
    q = query(
    valuation.code
    ).filter(
	valuation.code.in_(df)
    ...
    log.info("[ROIC_BIG] 按未分配利润降序取前%d只: %s" % (g.stock_num, str(final_list)))
    return final_list
def BM(context,choice):
    log.info("[BM] 输入股票数: %d" % len(choice))
    ...
    log.info("[BM] 过滤+排序后取前%d只: %s" % (g.stock_num, str(BM_list)))
    return BM_list
# 1-3 整体调整持仓
def monthly_adjustment(context):
    ...
    log.info("[调仓日 %s] 沪深300过滤后: %d 只" % (str(today.date()), len(B_stocks)))
    ...
    log.info("[调仓日 %s] 中小综指过滤后: %d 只" % (str(today.date()), len(S_stocks)))
    ...
    log.info("[调仓日 %s] 大盘动量样本(流通市值前20大): %s" % (str(today.date()), str(Blst)))
    ...
    log.info("[调仓日 %s] 小盘动量样本(流通市值前20小): %s" % (str(today.date()), str(Slst)))
    #
    ...
    log.info("[调仓日 %s] 动量: B_mean=%.2f%% S_mean=%.2f%%" % (str(today.date()), B_mean, S_mean))

    if B_mean > 10 or S_mean > 10:
        print('无敌好行情')
        log.info("[调仓日 %s] 无敌好行情" % str(today.date()))
        if B_mean > S_mean:
            print('开大')
            log.info("[调仓日 %s] 开大(无敌)" % str(today.date()))
            choice = B_stocks
        ...
        print('开小')
        log.info("[调仓日 %s] 开小" % str(today.date()))
        choice = S_stocks
        target_list = SMALL(context,choice)[:g.stock_num*3]
    else:
        print('开外盘')
        log.info("[调仓日 %s] 开外盘" % str(today.date()))
        target_list = g.foreign_ETF

    log.info("[调仓日 %s] 合并去重后目标列表(%d只): %s" % (str(today.date()), len(target_list), str(target_list)))
    target_list = filter_limitup_stock(context,target_list)
    target_list = filter_limitdown_stock(context,target_list)
    target_list = filter_paused_stock(target_list)
    log.info("[调仓日 %s] 过滤涨跌停/停牌后(%d只): %s" % (str(today.date()), len(target_list), str(target_list)))
    for stock in g.hold_list:
    ...
    if target_num > position_count:
        ...
        log.info("[调仓日 %s] 买入: 需买%d只, 每只%.0f元" % (str(today.date()), target_num-position_count, value))
