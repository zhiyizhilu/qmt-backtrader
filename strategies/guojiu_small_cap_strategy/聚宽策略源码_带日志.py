...
def get_stock_list(context):
    ...
    # ====== 新增日志：过滤后股票数量 ======
    log.info(f'[选股] 基本过滤后: {len(initial_list)} 只')
    ...
    # ====== 新增日志：国九条过滤后 + 市值排序前12 ======
    log.info(f'[选股] 国九条+市值过滤后: {len(df)} 只')
    for _, row in df.iterrows():
        log.info(f'  {row["code"]} | 市值: {row["market_cap"]:.2f}亿 | 归母净利: {row["np_parent_company_owners"]/1e8:.2f}亿 | 净利: {row["net_profit"]/1e8:.2f}亿 | 营收: {row["operating_revenue"]/1e8:.2f}亿')
    ...
    if len(final_list) == 0:
        ...
        result = [stock for stock in final_list if stock in g.hold_list or last_prices[stock][-1] <= g.highest]
        # ====== 新增日志：股价过滤后 ======
        filtered_by_price = [stock for stock in final_list if stock not in g.hold_list and last_prices[stock][-1] > g.highest]
        if filtered_by_price:
            log.info(f'[选股] 股价超限被过滤: {filtered_by_price}')
        log.info(f'[选股] 最终选股结果: {result}')
        return result
...
def adjust_stock_num(context):
    ...
    # ====== 新增日志：MA差值 ======
    log.info(f'[动态持股] 收盘={last_row["close"]:.2f}, MA{ma_para}={last_row["ma"]:.2f}, 差值={diff:.2f}')
